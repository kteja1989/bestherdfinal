    ...
    <div class="container w-full mx-auto pb-40">
        ...
        <!--Divider-->
            <div class="flex flex-row flex-wrap flex-grow mt-2">
                                                            ...
                                                            <td class="text-lg text-gray-600 font-medium px-6 py-4 whitespace-nowrap">
                                                                @hasanyrole('herdmanager|herdserum')
                                                                  @if($row->status == 'incomplete')
                                                                    <button wire:click="fetchSeraCollectedGoats('{{ $row->serum_id }}')" class="bg-orange-600 w-22 hover:bg-blue-800 text-white font-normal py-3 px-4 mx-3  rounded">Details</button>
                                                                  @else
                                                                    <button wire:click="fetchSeraCollectedGoats({{ $row->serum_id }})" class="bg-green-700 w-22 hover:bg-blue-800 text-white font-normal py-3 px-4 mx-3  rounded">Details</button>
                                                                  @endif
                                                                @endhasanyrole
                                                            </td>
                                                        </tr>
                ...
                @if($viewSerumForm)
                    @include('livewire.goats.editSerumForm')
                    @include('livewire.goats.serumDone')
                @endif
                <!-- panel opening/closing -->
            </div>
            <!--/ Console Content-->
